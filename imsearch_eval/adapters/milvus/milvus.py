...
class MilvusAdapter(VectorDBAdapter):
    ...
    def insert_data(
        ...
        **kwargs
        ...
        Args:
            flush: When True (default), flush the collection after inserts.
                Set False when streaming many chunks, then flush once at the end.
        """
        try:
            total_inserted = 0
    ...
    def flush_collection(self, collection_name: str) -> None:
        """Flush a Milvus collection (e.g. after streamed inserts with flush=False)."""
        try:
            self.milvus_client.flush(collection_name)
        except Exception as e:
            logging.debug(f"Flush of '{collection_name}' failed (continuing): {e}")
